      ...
      <article
        ...
        <div
          ...
          <div class="grid grid-cols-2 mt-4">
            <div v-for="(techno, techIndex) in project.technos" :key="techIndex"
              class="text-sm m-2 bg-indigo-500 py-1 min-w-1/2 rounded-xl text-slate-50">{{ techno }}</div>
          </div>
          <div class="flex flex-row justify-around">
            ...
            <a v-if="project.demo"
              class="bg-indigo-600 p-2 my-4 w-1/3 rounded-xl hover:scale-110 transition-transform duration-300 shadow-md text-slate-50"
    ...
    computed: {
      filteredProjects() {
        ...
        return this.projects.filter(project =>
          ...
          project.technos.some(tech => tech.toLowerCase().includes(query))
        );
      },
    },
    async mounted() {
